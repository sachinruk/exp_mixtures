...
%    \SetAlgoLined
    ...
    $p_1(y,\lambda_{old},\pi_{old},K)$\\
    \eIf{going up}{
    idx = randomly sample from $\{1,2,\cdots,k\}$\\
    $\lambda_{[k+1],\cdot}=\lambda_{old,\cdot}$\\
    $\lambda_{[k],idx}=\lambda_{1}Chain(last)$\\
    $\mu_1,\mu_2\sim  \,U(0,1)$\\
    $\lambda_{12}=\lambda_{old,idx}\frac{\mu_1}{1-\mu_1}$\\
    $\lambda_{22}=\lambda_{old,idx}\frac{1-\mu_1}{\mu_1}$\\
    $\pi=\mu_2$
    }{
	$\lambda_{12},\lambda_{22}=\lambda_{2}Chain(last)$\\
	$\lambda_{[k],idx} = \sqrt{\lambda_{12}\lambda_{22}}$\\
   	$\mu_1 = \frac{\lambda_{12}}{\lambda_{[k],idx}+\lambda_{12}}$
    }
    \vspace{-1cm}
    \begin{flalign*}
    ...
    \eIf{$A>u$}{
	    ...
	    \eIf{ state is 2}{
	    	$\lambda_1Chain.push(\lambda_{[k],idx})$\\
